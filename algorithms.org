...
Каждый узел будет содержать значение + цвет (в конс-паре) и два
указателя на потомков и один - на предка.

По умолчанию цвет узла черный, т.е. cdr конс-пары - это символ 'b (black),
иначе 'r (red).
...
#+BEGIN_SRC lisp :noweb yes
  (defun recolor-red (node)
    (setf (cdr (node-elt node)) 'r))

  (defun recolor-black (node)
    (setf (cdr (node-elt node)) 'b))

  ;; (setf test-node (make-node :elt (cons 2 'r)))
  ;; (recolor-black test-node)
#+END_SRC
...
#+BEGIN_SRC lisp :noweb yes
  (defun add-leafs (node)
    (setf (node-l node) (make-node)
          (node-r node) (make-node)))
#+END_SRC
...
#+NAME: recolor-black-dad-and-uncle
#+BEGIN_SRC lisp :noweb yes
...
#+NAME: rotate_if_uncle_black
#+BEGIN_SRC lisp :noweb yes
  (defun rotate-if-uncle-black-one-side (node)
    (let* ((dad (get-dad node))
           (uncle (get-uncle node))
           (grand-dad (get-grand-dad node))
...
#+BEGIN_SRC lisp :noweb yes
  ...
  (defun insert-in-red-black-tree-rec (tree parent elt)
    (if (null tree)
        (make-node :elt (cons elt 'b))
        (let ((node-value (get-value (node-elt tree))))
          (cond ((< elt node-value)
                 ;; левая ветка
                 (if (null (get-value (node-elt (node-l tree))))
                     ;; вставка
                     (progn
                       )
                     ;; иначе идем дальше
                     (insert-in-red-black-tree-rec (node-l tree) tree elt)

                 )
                 ((> elt node-value)
                  ;; правая ветка
                  (if (null (get-value (node-elt (node-r tree))))
                      ;; вставка
                      (progn
                        )
                      ;; иначе идем дальше
                      (insert-in-red-black-tree-rec (node-r tree) tree elt)
